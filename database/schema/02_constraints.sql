-- ============================================================
-- 02_constraints.sql
-- Phase 1 - Primary keys, foreign keys, unique constraints,
...
ALTER TABLE matches
    ADD CONSTRAINT fk_matches_sport
        FOREIGN KEY (sport_id)
            REFERENCES sports (sport_id);
...
ALTER TABLE tickets
    ADD CONSTRAINT fk_tickets_ticket_category
        FOREIGN KEY (ticket_category_id)
            REFERENCES ticket_categories (ticket_category_id);
...
ALTER TABLE reservations
    ADD CONSTRAINT fk_reservations_ticket
...
ALTER TABLE cancellation_policies
    ADD CONSTRAINT fk_cancellation_policies_match
        FOREIGN KEY (match_id)
            REFERENCES matches (match_id);

ALTER TABLE cancellation_policies
    ADD CONSTRAINT fk_cancellation_policies_ticket_category
        FOREIGN KEY (ticket_category_id)
            REFERENCES ticket_categories (ticket_category_id);
...
ALTER TABLE reports
    ADD CONSTRAINT fk_reports_reservation
        FOREIGN KEY (reservation_id)
            REFERENCES reservations (reservation_id);

ALTER TABLE reports
    ADD CONSTRAINT fk_reports_ticket
        FOREIGN KEY (ticket_id)
            REFERENCES tickets (ticket_id);

ALTER TABLE reports
    ADD CONSTRAINT fk_reports_report_category
        FOREIGN KEY (report_category_id)
            REFERENCES report_categories (report_category_id);

ALTER TABLE support_actions
    ADD CONSTRAINT fk_support_actions_report
        FOREIGN KEY (report_id)
            REFERENCES reports (report_id);

ALTER TABLE support_actions
    ADD CONSTRAINT fk_support_actions_reservation
        FOREIGN KEY (reservation_id)
            REFERENCES reservations (reservation_id);

ALTER TABLE support_actions
    ADD CONSTRAINT fk_support_actions_support_user
...
ALTER TABLE ticket_features
    ADD CONSTRAINT fk_ticket_features_ticket
        FOREIGN KEY (ticket_id)
            REFERENCES tickets (ticket_id);

ALTER TABLE ticket_features
    ADD CONSTRAINT fk_ticket_features_feature
        FOREIGN KEY (feature_id)
            REFERENCES features (feature_id);

ALTER TABLE football_details
    ADD CONSTRAINT fk_football_details_ticket
        FOREIGN KEY (ticket_id)
            REFERENCES tickets (ticket_id);

ALTER TABLE volleyball_details
    ADD CONSTRAINT fk_volleyball_details_ticket
        FOREIGN KEY (ticket_id)
            REFERENCES tickets (ticket_id);

ALTER TABLE basketball_details
    ADD CONSTRAINT fk_basketball_details_ticket
        FOREIGN KEY (ticket_id)
            REFERENCES tickets (ticket_id);
...
ALTER TABLE venues
    ADD CONSTRAINT chk_venues_total_capacity
...
ALTER TABLE matches
    ADD CONSTRAINT chk_matches_match_status
        CHECK (match_status IN ('SCHEDULED', 'POSTPONED', 'CANCELLED', 'FINISHED'));
...
ALTER TABLE tickets
    ADD CONSTRAINT chk_tickets_ticket_status
        CHECK (ticket_status IN ('AVAILABLE', 'RESERVED', 'SOLD', 'CANCELLED', 'UNAVAILABLE'));

ALTER TABLE reservations
    ADD CONSTRAINT chk_reservations_status
        CHECK (reservation_status IN ('PENDING', 'PAID', 'CANCELLED', 'EXPIRED'));

ALTER TABLE reservations
    ADD CONSTRAINT chk_reservations_expiry_time
        CHECK (expires_at > reserved_at);

ALTER TABLE reservations
    ADD CONSTRAINT chk_reservations_confirmed_time
        CHECK (confirmed_at IS NULL OR confirmed_at >= reserved_at);

ALTER TABLE reservations
    ADD CONSTRAINT chk_reservations_cancelled_time
...
ALTER TABLE payments
    ADD CONSTRAINT chk_payments_paid_time
        CHECK (paid_at IS NULL OR paid_at >= created_at);

ALTER TABLE cancellation_policies
    ADD CONSTRAINT chk_cancellation_policies_hours
        CHECK (hours_before_match >= 0);

ALTER TABLE cancellation_policies
    ADD CONSTRAINT chk_cancellation_policies_penalty
...
ALTER TABLE refunds
    ADD CONSTRAINT chk_refunds_penalty_amount
...
ALTER TABLE refunds
    ADD CONSTRAINT chk_refunds_processed_time
        CHECK (processed_at IS NULL OR processed_at >= requested_at);

ALTER TABLE reports
    ADD CONSTRAINT chk_reports_related_object_required
        CHECK (reservation_id IS NOT NULL OR ticket_id IS NOT NULL);

ALTER TABLE reports
    ADD CONSTRAINT chk_reports_status
        CHECK (report_status IN ('OPEN', 'IN_REVIEW', 'RESOLVED', 'REJECTED', 'CLOSED'));

ALTER TABLE support_actions
    ADD CONSTRAINT chk_support_actions_related_object_required
        CHECK (report_id IS NOT NULL OR reservation_id IS NOT NULL);

ALTER TABLE support_actions
    ADD CONSTRAINT chk_support_actions_action_type
        CHECK (action_type IN
               ('REVIEW_REPORT', 'CONFIRM_RESERVATION', 'CANCEL_RESERVATION', 'UPDATE_RESERVATION', 'RESPOND_TO_USER',
                'OTHER'));

ALTER TABLE otp_logs
    ADD CONSTRAINT chk_otp_logs_contact_required
        CHECK (user_id IS NOT NULL OR email IS NOT NULL OR phone_number IS NOT NULL);

ALTER TABLE otp_logs
    ADD CONSTRAINT chk_otp_logs_purpose
        CHECK (otp_purpose IN ('LOGIN', 'SIGNUP', 'PROFILE_UPDATE', 'PASSWORD_RESET'));

ALTER TABLE otp_logs
    ADD CONSTRAINT chk_otp_logs_status
        CHECK (otp_status IN ('PENDING', 'VERIFIED', 'EXPIRED', 'FAILED'));

ALTER TABLE otp_logs
    ADD CONSTRAINT chk_otp_logs_expiry_time
